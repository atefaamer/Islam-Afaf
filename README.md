...
>
> من نسخة الذكرى: القسم بقى **أرشيف قراءة فقط** من ناحية الضيوف — فورم الإرسال اتشال من الواجهة، وقاعدة البيانات (كوللكشن `wishes`) فضلت زي ما هي بكل رسائلها، والقراءة/ العرض شغالين في الوقت الحقيقي.
...
الصور اللي بتظهر في قسم «ذكرياتنا» في فولدر `public/photos/`، وأسماؤها مرتّبة: `01.jpg` و `02.jpg` و `03.jpg`.

**عايز تحط صور الفرح؟** سمّي صورك بنفس الأسماء (`01.jpg` / `02.jpg` / `03.jpg`) واستبدل اللي موجود — مش محتاج تلمس أي كود.

**عايز تزوّد صور أو تغيّر الكلام اللي تحت الصورة؟** ده في ملف واحد: `src/data/gallery.ts`

```ts
export const GALLERY: GalleryPhoto[] = [
  { src: './photos/01.jpg', caption: 'يوم ما وعدنا بعض', sub: 'THE PROMISE' },
  { src: './photos/02.jpg', caption: 'وأحنا راجعين', sub: 'ON OUR WAY' },
  { src: './photos/03.jpg', caption: 'قعدتنا المفضلة', sub: 'JUST US' },
  // ضيف سطر زي ده لأي صورة جديدة (بعد ما تحطها في public/photos/):
  // { src: './photos/04.jpg', caption: 'أول رقصة', sub: 'OUR FIRST DANCE' },
]
```

> خلي الصور مضغوطة (أقل من ~500 كيلوبايت للصورة) عشان الموقع يفتح بسرعة — أي برنامج أو موقع ضغط صور بيعمل الغرض.

**الصور الأصلية** اللي كانت في المشروع (بحجمها الكبير) في فولدر `originals/` في جذر المشروع — مش بتترفع مع الموقع.
...
public/
  photos/                     الصور اللي بتظهر في «ذكرياتنا» — هنا بتحط صور جديدة
  music.mp3                    الموسيقى
  og-image.jpg                  الصورة اللي بتظهر لما حد يشارك اللينك
originals/                    نسخ الصور الأصلية (مش بتترفع مع الموقع)
```
...
- **حائط الرسائل** بقى **دفتر ذكرى مقفول** (OUR GUESTBOOK / «كلمات من أحبّائنا»): كل الرسايل القديمة محفوظة ومعروضة زي ما هي بالكامل (أسماء/نصوص/تواريخ/ردود/تثبيت)، وفورم الإرسال اتشال من الموقع — مفيش إرسال جديد من الضيوف، والقسم بيختتم بسطر باهت: «The guestbook is closed. The memories remain.» العروسين لسه يقدروا يسجّلوا دخول ويردّوا ويثبتوا (زرار الدبوس) ويحذفوا.
- عنوان الموقع ووصفه (اللي بيظهروا في واتساب/فيسبوك/جوجل) اتحدّثوا، واتضافت صورة مشاركة `public/og-image.jpg`.
- **النسخة المختصرة:** المشاهد الطويلة اتشالت عشان التجربة تبقى قصيرة ومركّزة — اتشال مشهد الخريطة/المكان (`LocationInteractive`)، وجدول التفاصيل WHEN/AT/WHERE (`InfoEditorial`)، وتحويل التاريخ/المكان (`DateMorph`)، والصور الثلاث السينمائية (`ImageSequence` + `ImagePanel`) لأنها كانت مكررة مع الألبوم. اللي فاضل:

  ```
  الاسمين ← الآية ← ذكرياتنا (الصور) ← كلام الضيوف ← بقالنا كام يوم ← الختام
  ```

  ملفات المشاهد المشالة موجودة في تاريخ الجيت (آخر كوميت قبل «النسخة المختصرة») لو حبيت ترجّع أي واحد منها.
...
> الأسماء الموجودة حالياً في الملف (يوم ما وعدنا بعض / وأحنا راجعين / قعدتنا المفضلة) مجرد اقتراحات — غيّرها لأي حاجة تفتكروها.
>
> الصور الأصلية (بحجمها الكبير) اتنقلت لفولدر `originals/` في جذر المشروع — مش بتترفع مع الموقع، بس محفوظة عندك.
